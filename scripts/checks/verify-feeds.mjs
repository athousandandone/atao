...
function pageUrl(htmlFile) {
  return (
    origin + "/" + htmlFile.replace(/index\.html$/, "").replace(/^\/$/, "")
  );
}
...
export async function check() {
  ...
  const articlePages = pages.filter(
    f =>
      /^blog\/[^/]+\/index\.html$/.test(f) && !/^blog\/(archive|tags)\//.test(f)
  );
  ...
  if (!existsSync(join(DIST, "robots.txt"))) {
    fail(
      "robots.txt",
      "robots.txt missing from dist/",
      "public/robots.txt should pass through the build."
    );
  } else {
    ...
    if (sitemapLines.length !== 1) {
      fail(
        "robots.txt",
        `expected exactly one Sitemap line, found ${sitemapLines.length}`,
        "robots.txt carries a single Sitemap directive."
      );
    } else {
      ...
      if (!sitemapUrl.startsWith(origin + "/")) {
        fail(
          "robots.txt",
          `Sitemap URL "${sitemapUrl}" is not at the site origin ${origin}`,
          "the Sitemap directive must use the configured origin."
        );
      } else if (!existsSync(join(DIST, sitemapUrl.slice(origin.length + 1)))) {
        fail(
          "robots.txt",
          `Sitemap URL "${sitemapUrl}" names a file the build did not emit`,
          "point the directive at the sitemap entry point as emitted — no assumed filenames."
        );
      }
    }
  }
  ...
  if (!existsSync(join(DIST, indexFile))) {
    fail(
      indexFile,
      "sitemap entry point missing from dist/",
      "the sitemap integration should emit sitemap-index.xml."
    );
  } else {
    ...
    for (const part of parts) {
      if (!part.startsWith(origin + "/")) {
        fail(
          indexFile,
          `sitemap part "${part}" is not at the site origin`,
          "sitemap URLs must use the configured origin."
        );
        continue;
      }
      ...
      if (!existsSync(join(DIST, file))) {
        fail(
          indexFile,
          `sitemap part "${part}" names a file the build did not emit`,
          "every referenced sitemap file must exist."
        );
        continue;
      }
    ...
    for (const url of urls) {
      if (
        ...
        fail(
          indexFile,
          `sitemap URL "${url}" is not in origin trailing-slash form`,
          "canonical form is origin + path + trailing slash."
        );
      }
      if (!expected.has(url)) {
        fail(
          indexFile,
          `sitemap lists "${url}" which is not an emitted page`,
          "the sitemap must cover exactly the emitted routes (404 excluded)."
        );
      }
    }
    for (const url of expected) {
      if (!urls.includes(url)) {
        fail(
          indexFile,
          `emitted page "${url}" is missing from the sitemap`,
          "the sitemap must cover every expected route."
        );
      }
    }
  }
  ...
  if (!existsSync(join(DIST, feedFile))) {
    fail(
      feedFile,
      "feed missing from dist/",
      "src/pages/blog/rss.xml.js should emit the feed."
    );
  } else {
    ...
    if (channelLink !== origin + "/") {
      fail(
        feedFile,
        `channel link "${channelLink}" is not the origin root`,
        `expected ${origin}/.`
      );
    }
    ...
    for (const link of itemLinks) {
      if (!link.startsWith(origin + "/blog/") || !link.endsWith("/")) {
        fail(
          feedFile,
          `item link "${link}" is not in origin trailing-slash form`,
          "feed links carry the canonical URL form."
        );
      }
      if (!expected.has(link)) {
        fail(
          feedFile,
          `feed lists "${link}" which is not an emitted article page`,
          "feed items must be exactly the published articles."
        );
      }
    }
    for (const url of expected) {
      if (!itemLinks.includes(url)) {
        fail(
          feedFile,
          `emitted article "${url}" is missing from the feed`,
          "every published article belongs in the feed."
        );
      }
    }
  }
  ...
  for (const file of metaScope) {
    ...
    for (const node of walkNodes(parseHtmlFile(`${DIST}/${file}`))) {
      ...
      if (node.tagName === "meta" && attr("name") === "description")
        counts.description += 1;
      if (node.tagName === "link" && attr("rel") === "canonical")
        counts.canonical.push(attr("href"));
      if (node.tagName === "meta" && attr("property") === "og:url")
        counts.ogUrl.push(attr("content"));
    }
    if (counts.title !== 1)
      fail(
        file,
        `expected exactly one <title>, found ${counts.title}`,
        "every page carries one title."
      );
    if (counts.description !== 1)
      fail(
        file,
        `expected exactly one meta description, found ${counts.description}`,
        "every page carries one description."
      );
    if (file === "404.html") {
      if (counts.canonical.length !== 0 || counts.ogUrl.length !== 0) {
        fail(
          file,
          "the 404 must not claim a canonical or og:url",
          "an error page has no canonical address."
        );
      }
    } else {
      ...
      if (counts.canonical.length !== 1 || counts.canonical[0] !== expected) {
        fail(
          file,
          `expected one canonical "${expected}", found [${counts.canonical.join(", ")}]`,
          "each blog page canonicalises to its own trailing-slash URL."
        );
      }
      if (counts.ogUrl.length !== 1 || counts.ogUrl[0] !== expected) {
        fail(
          file,
          `expected one og:url "${expected}", found [${counts.ogUrl.join(", ")}]`,
