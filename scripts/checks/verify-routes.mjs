...
// Slice 5: archive, the root-level about and the fixture tags' routes.
const EXPECTED = [
  "index.html",
  "404.html",
  "about/index.html",
  ...
  "blog/tags/food/index.html",
  "blog/tags/product/index.html",
  "blog/tags/travel/index.html"
];
...
export async function check() {
  ...
  for (const route of EXPECTED) {
    ...
    if (!existsSync(path) || statSync(path).size === 0) {
      violations.push({
        file: `${DIST}/${route}`,
        line: 0,
        pattern: "expected route file missing or empty",
        excerpt: "",
        hint: "the route manifest in scripts/checks/verify-routes.mjs lists every route the build must emit."
      });
    }
  }
  ...
  if (existsSync(join(DIST, "server"))) {
    violations.push({
      file: `${DIST}/server`,
      line: 0,
      pattern: "server output in a static build",
      excerpt: "",
      hint: "output must remain fully static; no adapter or dynamic rendering is permitted."
    });
  }
      ...
      for (const attr of node.attrs ?? []) {
        ...
        for (const url of urls) {
          ...
          if (
            ...
            statSync(join(DIST, target)).isDirectory()
          ) {
