...
function scanMarkup(file, path, violations) {
  ...
  for (const node of walkNodes(doc)) {
    ...
    if (tag === "astro-island") {
      add(
        node,
        "astro-island element (island hydration)",
        "a client:* directive or framework integration produced hydration output; none is permitted."
      );
    }
    if (tag === "link") {
      ...
      if (rel.includes("modulepreload")) {
        add(
          node,
          'rel="modulepreload" (module preloading)',
          "module preloading only exists to serve executable modules; none may ship."
        );
      }
    }
    for (const attr of node.attrs ?? []) {
      ...
      if (/^on[a-z]+$/.test(name)) {
        add(
          node,
          `inline event-handler attribute (${name})`,
          "event-handler attributes are executable JavaScript; remove them."
        );
      }
      if (
        ...
        add(
          node,
          `javascript: URL in ${name}`,
          "javascript: URLs are executable; use a real target."
        );
      }
    }
  }
  ...
  for (const file of listFiles(DIST)) {
    ...
    if (/\.(js|mjs|cjs)$/i.test(file)) {
      violations.push({
        file: path,
        line: 0,
        pattern: "browser JavaScript file",
        excerpt: "",
        hint: "no JavaScript files may exist in generated output."
      });
    } else if (/\.map$/i.test(file)) {
      violations.push({
        file: path,
        line: 0,
        pattern: "source map — evidence of an unexpected toolchain path",
        excerpt: "",
        hint: "no source maps are expected; find what emitted it."
      });
    } else if (/\.wasm$/i.test(file)) {
      violations.push({
        file: path,
        line: 0,
        pattern: "WebAssembly — an unexpected browser runtime artefact",
        excerpt: "",
        hint: "no browser runtime artefacts are expected; find what emitted it."
      });
